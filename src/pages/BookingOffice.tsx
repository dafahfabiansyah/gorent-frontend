...
const BookingOffice = () => {
  ...
  useEffect(() => {
    apiService
      ...
      .then((response) => {
        ...
        const generateUniqueCode = Math.floor(100 + Math.random() * 100);
      ...
      .catch((error: unknown) => {
        if (axios.isAxiosError(error)) {
          console.error(`ini error ${error.message}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log("validation data from ....");
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col shrink-0 w-[400px] h-fit rounded-[20px] border border-[#E0DEF7] p-[30px] gap-[30px] bg-white">
          ...
          >
            <span>{isLoading ? "Loading" : "I’ve Made The Paymentd"}</span>
          </button>
        </div>
      </form>
    </>
